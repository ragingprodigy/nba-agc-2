...
exports.papers = function (req, res) {
    Session.find({})
        ...
        var papers = _.flatten(_.pluck(_.filter(sessions, function (s) {
            return s.papers.length;
        }), 'papers'));
        res.json(papers);
    });
};
...
exports.removeQuestion = function (req, res) {
    ...
    Session.findById(req.params.id).populate('speakers').exec(function (err, session) {
        ...
        var isMine = _.find(session.questions, function (q) {
            return q._id.toString() == req.params.questionId && q.owner == req.user;
        });

        if (isMine) {
            session.questions = _.filter(session.questions, function (q) {
                return q._id.toString() !== req.params.questionId;
            });
            session.save(function (err) {
                if (err) {
                    return handleError(res, err);
    ...
    Session.findById(req.params.id, function (err, session) {
        ...
        if (moment(session.rating_start).isBefore(new Date())) {
            ...
            Attendee.findOne({user: new ObjectId(req.user)}, function (err, regDetail) {
                ...
                }, _.pick(req.body, ['comment', 'score']), function (err, userRating, created) {
                    if (err) {
                        return handleError(res, err);
                    }

                    if (!created) {
                        return res.status(409).json({message: 'You can only submit a rating once'});
                    }
                    else {
            ...
            return res.status(400).json({
                message: 'Rating is not open yet. Rating starts: ' +
                voting_starts
            });
        }
    });
        ...
        .exec(function (err, session) {
            ...
            if (moment(session.end_time).isAfter(new Date())) {
                ...
                Attendee.find({user: new ObjectId(req.user)})
                    ...
                    var sameTimeSession = _.find(sameTime, function (a) {
                        return moment(a.session.start_time).isSame(session.start_time);
                    });
        ...
        .exec(function (err, session) {
            ...
            if (moment(session.end_time).isAfter(new Date())) {
                Attendee.findOne({user: new ObjectId(req.user), session: new ObjectId(req.params.id)}, function (err, record) {
                    ...
                    if (!record) {
                        return res.status(401).json({message: 'You have not registered to attend this session.'});
                    }
