...
exports.sendBankPaySuccessMail = function(registration, next) {
    ...
    var newMessage = message;
    newMessage.html = emailData.replace('{{code}}', (registration.regCode+'-'+registration.conferenceFee)).replace('{{fee}}', registration.conferenceFee).replace('{{amount}}', registration.bankDeposit).replace('{{teller}}', registration.bankTeller).replace('{{date}}', registration.bankDatePaid);
